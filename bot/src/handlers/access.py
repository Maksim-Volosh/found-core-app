...
async def verify_payment_status_handler(
    ...
    if access_data and access_data["allowed"]:
        try:
            ...
            if direction_response["requires_screening"] is True:
                direction_access_response = (
                    await container.direction_service.get_direction_access(
                        backend_user_id, telegram_chat_id
                    )
                )
...
async def on_user_join(event: ChatMemberUpdated, bot: Bot):
    ...
    try:
        ...
        if not backend_user_id:
            ...
            is_allowed_data = await container.access_service.check_main_access(
                backend_user_id
            )
