...
from app.infrastructure.payment_providers.stripe_provider import StripePaymentProvider
...
class Container:
    ...
    def get_user_auth_use_case(self):
        return UserAuthUseCase(
            user_repo=self.user_repo(), subscription_repo=self.subscription_repo()
        )
    ...
    def get_admin_subscription_use_case(self):
        return AdminSubscriptionUseCase(
            user_repo=self.user_repo(), subscription_repo=self.subscription_repo()
        )

    def get_clear_expired_subscriptions_use_case(self):
        return ClearExpiredSubscriptionsUseCase(
            subscription_repo=self.subscription_repo(),
            direction_repo=self.direction_repo(),
            user_repo=self.user_repo(),
            bot_service=self.get_bot_service(),
        )

    def get_send_subscription_reminders_use_case(self):
        return SendSubscriptionRemindersUseCase(
            subscription_repo=self.subscription_repo(),
            user_repo=self.user_repo(),
            bot_service=self.get_bot_service(),
        )
    ...
    def get_create_direction_use_case(self):
        return CreateDirectionUseCase(
            direction_repo=self.direction_repo(), user_repo=self.user_repo()
        )
