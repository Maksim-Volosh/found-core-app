...
from app.infrastructure.mappers.direction import (
    DirectionMapper,
    UserDirectionAccessMapper,
)
...
class SQLAlchemyDirectionRepository(IDirectionRepository):
    ...
    async def get_directions(self) -> list[DirectionEntity] | None:
        ...
        if direction_models is None:
            return None
        return [
            DirectionMapper.from_model(direction_model)
            for direction_model in direction_models
        ]
    ...
    async def change_user_direction_access(
        ...
        q = select(UserDirectionAccess).where(
            ...
            UserDirectionAccess.telegram_chat_id
            == user_direction_access.telegram_chat_id,
        )
        user_direction_access_model = await self.session.scalar(q)
        if user_direction_access_model is None:
            return None
        user_direction_access_model.screening_status = (
            user_direction_access.screening_status
        )
        await self.session.commit()
        return UserDirectionAccessMapper.from_model(user_direction_access_model)
