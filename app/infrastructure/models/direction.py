...
class Direction(Base):
    ...
    directions: Mapped[List["UserDirectionAccess"]] = relationship(
        "UserDirectionAccess", back_populates="direction", cascade="all, delete-orphan"
    )
...
class UserDirectionAccess(Base):
    ...
    user_direction_access_id: Mapped[int] = mapped_column(
    ...
    telegram_chat_id: Mapped[int] = mapped_column(
        BigInteger,
        ForeignKey("direction.telegram_chat_id", ondelete="CASCADE"),
        index=True,
    )
    ...
    direction: Mapped["Direction"] = relationship(
        "Direction", back_populates="directions"
    )
