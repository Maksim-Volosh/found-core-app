...
from app.domain.interfaces import IBotService, ISubscriptionRepository, IUserRepository
...
class SendSubscriptionRemindersUseCase:
    ...
    async def execute(self) -> None:
        ...
        subs_7 = await self.subscription_repo.get_subs_for_7_days_reminder(
            target_7_days
        )
        ...
        logger.info(
            f"Отправлено напоминаний для {len(subs_7)} подписок, срок действия которых истекает через <= 7 дней."
        )

        target_3_days = now + timedelta(days=3)
        subs_3 = await self.subscription_repo.get_subs_for_3_days_reminder(
            target_3_days
        )
        ...
        logger.info(
            f"Отправлено напоминаний для {len(subs_3)} подписок, срок действия которых истекает через <= 3 дней."
        )
